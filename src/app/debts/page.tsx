...
export default function DebtsPage() {
  ...
  const handleAddPayment = async (
    ...
    if (numericAmount > remaining) {
      toast.error("Nominal melebihi sisa hutang/piutang!");
      return;
    }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!form.personName.trim()) {
      toast.error("Nama orang wajib diisi!");
      return;
    }
    ...
    <PageContainer className="min-h-screen">
      ...
      <header className="space-y-1 sticky top-0 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md pb-4 z-10 -mx-6 px-6 pt-4 border-b border-zinc-100 dark:border-zinc-800">
        ...
        <p className="text-sm text-zinc-500">
          Lacak uang yang dipinjam dan yang dipinjamkan
        </p>
      </header>
        ...
        <div>
          ...
          <p
            className={cn(
              ...
              activeTab === "PIUTANG" ? "text-emerald-500" : "text-red-500",
            )}
          >
      ...
      {isAdding && (
        <form
          onSubmit={handleAdd}
          className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4 space-y-3 shadow-sm"
        >
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center bg-zinc-50 dark:bg-zinc-900/50 rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800">
            ...
            <p className="text-zinc-500 font-medium">
              Tidak ada catatan {activeTab.toLowerCase()}
            </p>
          </div>
        ) : (
          debts.map((debt) => {
            ...
            const totalPaid =
              debt.payments?.reduce((acc, p) => acc + p.amount, 0) || 0;
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={cn(
                          ...
                          debt.isPaid && "line-through text-zinc-400",
                        )}
                      >
                        {debt.personName}
                      </p>
                ...
                <div className="pt-3 border-t border-zinc-100 dark:border-zinc-800 flex flex-col gap-3">
                  ...
                  {!debt.isPaid &&
                    ...
                    (addingPaymentId === debt.id ? (
                      <form
                        onSubmit={(e) =>
                          handleAddPayment(e, debt.id, remaining)
                        }
                        className="bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 space-y-2 shadow-sm"
                      >
                        <div className="flex gap-2">
                          <div className="relative flex-1">
                            ...
                            <Input
                              ...
                              className="pl-7 text-xs h-8 rounded-lg"
                              placeholder="0"
                            />
                          </div>
                          <Input
                            ...
                            className="w-32 text-xs h-8 rounded-lg"
                          />
                        </div>
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() => setAddingPaymentId(null)}
                          >
                            Batal
                          </Button>
                          <Button
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full text-xs rounded-xl h-8 text-zinc-600 dark:text-zinc-400"
                        onClick={() => {
